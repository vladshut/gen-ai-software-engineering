...
The pipeline has **three runtime agents**, each a deterministic function (no LLM
...
- Resets `shared/`, loads `sample-transactions.json`, and runs each transaction
  through validator → fraud → settlement, writing a message at every hop.
- Masks PII (`****1234`) in all console logging via `safe_log_view` /
  `mask_account`.
- Writes `shared/results/_run_summary.json` (fraud-decision counts, settlement
  counts, settled USD total, per-transaction outcomes).
